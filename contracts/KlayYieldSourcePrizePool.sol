...
/**
 * @title  KlayTogether V1 KlayYieldSourcePrizePool
 * @author KlayTogether Inc Team
 * @notice The Klay Yield Source Prize Pool V1 uses KLAY as base asset and KlaySwap as a yield source to generate prizes.
 *         Funds that are deposited into the prize pool are then deposited into KlaySwap to generate yield.
 */
contract KlayYieldSourcePrizePool is PrizePool {
    ...
    /// @notice Deploy the Klay Yield Source Prize Pool
    ...
    constructor(
        ...
        require(
            address(klayswapPool.token()) == address(ZERO_ADDRESS),
            "KlayYieldSourcePrizePool/invalid-token-pool"
        );
        emit Deployed(address(ZERO_ADDRESS), address(klayswapPool));
    }
    ...
    function _supply(uint256 _amount) internal override {
        klayswapPool.depositKlay{value: _amount}();
    }
    ...
    function _award(uint256 _basePrize) internal override returns (uint256) {
        ...
        uint256 beforeKspExchange = address(this).balance;
        ...
        return address(this).balance - beforeKspExchange;
    }
